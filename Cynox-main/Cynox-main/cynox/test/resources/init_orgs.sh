...
do
  ...
  do
    ...
    curl -s -w '%{http_code}\n' -o /dev/null -uadmin:secret http://127.0.0.1:9000/api/v1/user -H 'Content-type: application/json' -d '{"login":"'$ORG$USER'@cynox.local","name":"'$ORG$USER'","profile":"'${PROFILES[$USER]}'","organisation":"'$ORG'"}'
    echo -n "Set password $ORG$USER to user $ORG$USER ... "
    curl -s -w '%{http_code}\n' -o /dev/null -uadmin:secret http://127.0.0.1:9000/api/v1/user/$ORG${USER}@cynox.local/password/set -H 'Content-type: application/json' -H "X-Organisation: $ORG" -d '{"password": "'$ORG$USER'"}'
  done
  for I in $(seq 1 10)
  do
