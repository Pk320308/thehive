...
import org.thp.cynox.controllers.v0.AlertCtrl
import org.thp.cynox.dto.v0.{InputAlert, OutputAlert}
import org.thp.cynox.services.{AlertSrv, AuditSrv, OrganisationSrv, UserSrv}
  ...
  "alert created trigger" should {
    "be properly triggered on alert creation" in testApp { app =>
      app[Database].roTransaction { implicit graph =>
        ...
        val user2 = app[UserSrv].getOrFail(EntityName("certadmin@cynox.local"))
        val user1 = app[UserSrv].getOrFail(EntityName("certuser@cynox.local"))
