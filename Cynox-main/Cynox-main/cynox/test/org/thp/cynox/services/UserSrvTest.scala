...
class UserSrvTest extends PlaySpecification with TestAppBuilder {
  implicit val authContext: AuthContext = DummyUserSrv(userId = "admin@cynox.local").getSystemAuthContext
  ...
  "user service" should {
    ...
    "create and get an user by his login" in testApp { app =>
      app[Database].transaction { implicit graph =>
        app[UserSrv].createEntity(
          User(
            login = "getbylogintest@cynox.local",
    ...
    "deduplicate users in an organisation" in testApp { app =>
      ...
      db.tryTransaction { implicit graph =>
        val certadmin = userSrv.get(EntityName("certadmin@cynox.local")).head
        ...
        val userCount = userSrv.get(EntityName("certadmin@cynox.local")).organisations.get(EntityName("cert")).getCount
