...
class AttachmentSrvTest extends PlaySpecification with TestAppBuilder {
  implicit val authContext: AuthContext = DummyUserSrv(userId = "certuser@cynox.local", organisation = "cert").getSystemAuthContext
...
object WithFakeScalligraphFile {
  ...
  def apply[A](body: Files.TemporaryFile => A): A = {
    val tempFile = File.createTempFile("cynox-", "-test")
