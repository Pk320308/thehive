...
import org.thp.cynox.services.ShareOps._
...
class CaseSrvTest extends PlaySpecification with TestAppBuilder {
  implicit val authContext: AuthContext =
    DummyUserSrv(userId = "certuser@cynox.local", organisation = "cert", permissions = Profile.analyst.permissions).authContext
  ...
  "case service" should {
    ...
    "get a case without impact status" in testApp { app =>
      app[Database].roTransaction { implicit graph =>
        ...
        val expected = RichCase(
          ...
          assignee = Some("certuser@cynox.local"),
          Nil,
          Set(
    ...
    "get a case with impact status" in testApp { app =>
      app[Database].roTransaction { implicit graph =>
        ...
        richCase._createdBy must_=== "system@cynox.local"
      }
    }
    ...
    "get a case with custom fields" in testApp { app =>
      app[Database].roTransaction { implicit graph =>
        val richCase =
          app[CaseSrv].get(EntityName("3")).richCase(DummyUserSrv(userId = "socuser@cynox.local", organisation = "soc").authContext).head
        ...
        richCase.assignee     must beSome("socuser@cynox.local")
    ...
    "assign/unassign a case" in testApp { app =>
      ...
      app[Database].tryTransaction(implicit graph =>
        app[CaseSrv].assign(c8, app[UserSrv].get(EntityName("certuser@cynox.local")).getOrFail("Case").get)
      ) must beSuccessfulTry
      checkAssignee(beTrue)
    }
    ...
    "forbid correctly case access" in testApp { app =>
      app[Database].roTransaction { implicit graph =>
        app[CaseSrv]
          .get(EntityName("1"))
          .can(Permissions.manageCase)(DummyUserSrv(userId = "certro@cynox.local", organisation = "cert").authContext)
          .exists must beFalse
      }
    }
