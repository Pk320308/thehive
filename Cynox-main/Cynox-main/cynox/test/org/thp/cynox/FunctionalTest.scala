...
//import org.thp.cynox.client.CynoxClient
//import org.thp.cynox.controllers.v1.{TestCase, TestUser}
//import org.thp.cynox.dto.v1._
...
////      |  storage.directory: /tmp/cynox-test.db
////      |}
////      |storage {
////      |  provider: localfs
////      |  localfs.location: /tmp
...
//
//    s"[${dbConfig.get[String]("db.provider")}] Cynox" should {
//      lazy val app = server.application
//
//      implicit lazy val ec: ExecutionContext = app.injector.instanceOf[ExecutionContext]
//      implicit lazy val ws: WSClient         = app.injector.instanceOf[WSClient]
//      lazy val client                        = new CynoxClient(s"http://127.0.0.1:${server.runningHttpPort.get}")
//
...
//            new CynoxModule(Environment.simple(), dbConfig),
//            new SimpleModule(
...
//
//        "get admin user" in {
//          val asyncResp = client.user.get("admin@cynox.local")
//          adminUser = TestUser(await(asyncResp))
//          adminUser.login must_=== "admin@cynox.local"
//        }
//
//        "create new user" in {
//          val asyncResp = client.user.create(InputUser("toom@cynox.local", "Thomas", Some("secret"), "analyst"))
//          user2 = TestUser(await(asyncResp))
//          val expected = TestUser("toom@cynox.local", "Thomas", "analyst", Set("manageAlert", "manageCase", "manageTask"), "admin")
...
//          val wrongAuth = PasswordAuthentication("admin@cynox.local", "nopassword")
...
//          val asyncResp =
//            client.user.create(InputUser("testAdmin@cynox.local", "Admin user in test organisation", Some("secret"), "admin", Some("test")))
//          user3 = TestUser(await(asyncResp))
//          val expected = TestUser("testAdmin@cynox.local", "Admin user in test organisation", "admin", Permissions.all.map(_.toString), "test")
//          user3 must_== expected
//        }
//      }
//
//      {
//        implicit val auth: Authentication = PasswordAuthentication("testAdmin@cynox.local", "secret")
//
...
//            client
//              .`case`
//              .create(InputCase("test case", "Case in test organisation", severity = Some(1), pap = Some(1), user = Some("testAdmin@cynox.local")))
...
//            pap = 1,
//            status = "Open",
//            user = Some("testAdmin@cynox.local")
//          )
//
...
//            _id = alert._id,
//            _createdBy = "testAdmin@cynox.local",
