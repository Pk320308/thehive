...
import org.thp.cynox.dto.v0.{InputOrganisation, OutputOrganisation}
...
class OrganisationCtrlTest extends PlaySpecification with TestAppBuilder {
  "organisation controller" should {
    ...
    "get an organisation" in testApp { app =>
      val request = FakeRequest("GET", s"/api/v0/organisation/cert").withHeaders("user" -> "certuser@cynox.local")
    ...
    "refuse to get a invisible organisation" in testApp { app =>
      val request = FakeRequest("GET", s"/api/v0/organisation/FIXME").withHeaders("user" -> "certuser@cynox.local")
