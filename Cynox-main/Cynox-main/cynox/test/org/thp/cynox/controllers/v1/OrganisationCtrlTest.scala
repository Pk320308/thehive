...
import org.thp.cynox.dto.v1.{InputOrganisation, OutputOrganisation}
import org.thp.cynox.models.Organisation
import org.thp.cynox.services.OrganisationSrv
...
class OrganisationCtrlTest extends PlaySpecification with TestAppBuilder {
  "organisation controller" should {
    ...
    "get an organisation" in testApp { app =>
      val request = FakeRequest("GET", s"/api/v1/organisation/cert").withHeaders("user" -> "certuser@cynox.local")
    ...
    "get a visible organisation" in testApp { app =>
      val request = FakeRequest("GET", s"/api/v1/organisation/soc").withHeaders("user" -> "certuser@cynox.local")
    ...
    "refuse to get a invisible organisation" in testApp { app =>
      val request = FakeRequest("GET", s"/api/v1/organisation/${Organisation.administration.name}").withHeaders("user" -> "certuser@cynox.local")
