...
import org.thp.cynox.dto.v0.OutputTask
import org.thp.cynox.models._
import org.thp.cynox.services.TaskOps._
import org.thp.cynox.services.{CaseSrv, TaskSrv}
...
class TaskCtrlTest extends PlaySpecification with TestAppBuilder {
  "task controller" should {
    "list available tasks and get one task" in testApp { app =>
      ...
      val request    = FakeRequest("GET", s"/api/case/task/$taskId").withHeaders("user" -> "certuser@cynox.local")
      ...
      val expected = TestTask(
        ...
        owner = Some("certuser@cynox.local"),
        startDate = None,
        status = "Waiting",
        group = Some("group1"),
        endDate = None,
    ...
    "patch a task" in testApp { app =>
      ...
      val request = FakeRequest("PATCH", s"/api/case/task/$taskId")
        ...
        .withJsonBody(Json.parse("""{"title": "new title task 1", "owner": "certuser@cynox.local", "status": "InProgress"}"""))
    ...
    "create a new task for an existing case" in testApp { app =>
      ...
      val requestGet = FakeRequest("GET", s"/api/case/task/${resultTaskOutput.id}").withHeaders("user" -> "certuser@cynox.local")
