...
import org.thp.cynox.services.AttachmentSrv
...
class Router @Inject() (entrypoint: Entrypoint, vfs: VFS, db: Database, attachmentSrv: AttachmentSrv) extends SimpleRouter {
  ...
  def dav(path: String): Action[AnyContent] =
    entrypoint("dav")
      ...
      .authPermittedRoTransaction(db, Permissions.accessCynoxFS) { implicit request => implicit graph =>
  ...
  def downloadFile(id: String): Action[AnyContent] =
    entrypoint("download attachment")
      .authPermittedRoTransaction(db, Permissions.accessCynoxFS) { request => implicit graph =>
