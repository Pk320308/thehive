...
import org.thp.cynox.services.ProfileOps._
import org.thp.cynox.services.RoleOps._
...
class UserSrv @Inject() (
  ...
  def checkUser(user: User): Try[User] = {
    ...
    if (fullUserNameRegex.matcher(login).matches() && login != "system@cynox.local") Success(user.copy(login = login))
