...
import org.thp.cynox.services.LocalUserSrv
...
@Singleton
class CynoxSchemaDefinition @Inject() extends Schema with UpdatableSchema {
  ...
  val operations: Operations = Operations("cynox")
    ...
    //=====[release 4.0.1]=====
    .updateGraph("Add accessCynoxFS permission to analyst and org-admin profiles", "Profile") { traversal =>
      traversal
        .unsafeHas("name", P.within("org-admin", "analyst"))
        .onRaw(_.property(Cardinality.set: Cardinality, "permissions", "accessCynoxFS", Nil: _*)) // Nil is for disambiguate the overloaded methods
        .iterate()
      Success(())
    }
    //=====[release 4.0.2]=====
      ...
      db.tryTransaction { implicit graph =>
        ...
        graph.V("Organisation").unsafeHas("name", P.neq("admin")).foreach { o =>
          ...
          if (!hasFreetagsTaxonomy) {
            ...
            taxoVertex.property("_createdBy", "system@cynox.local")
...
//    }
  ...
  val reflectionClasses = new Reflections(
    new ConfigurationBuilder()
      .forPackages("org.thp.cynox.models")
