...
class Connector @Inject() (appConfig: ApplicationConfig, system: ActorSystem, mat: Materializer, implicit val ec: ExecutionContext)
    extends CynoxConnector {
  override val name: String = "misp"

  val clientsConfig: ConfigItem[Seq[CynoxMispClientConfig], Seq[CynoxMispClient]] =
    appConfig.mapItem[Seq[CynoxMispClientConfig], Seq[CynoxMispClient]]("misp.servers", "", _.map(new CynoxMispClient(_, mat)))
  def clients: Seq[CynoxMispClient] = clientsConfig.get
