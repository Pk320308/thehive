...
import org.thp.cynox.services.{AlertSrv, AttachmentSrv, CaseSrv, OrganisationSrv}
...
class MispExportSrv @Inject() (
  ...
  def getMispClient(mispId: String): Future[CynoxMispClient] =
    connector
      .clients
      .find(_.name == mispId)
      .fold[Future[CynoxMispClient]](Future.failed(NotFoundError(s"MISP server $mispId not found"))) {
  ...
  def createEvent(client: CynoxMispClient, `case`: Case with Entity, attributes: Seq[Attribute], extendsEvent: Option[String])(implicit
      ec: ExecutionContext
  ): Future[String] = {
    val mispTags =
  ...
  def createAlert(client: CynoxMispClient, `case`: Case with Entity, eventId: String)(implicit
      graph: Graph,
  ...
  def canExport(client: CynoxMispClient)(implicit authContext: AuthContext): Boolean =
