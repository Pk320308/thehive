...
class MispImportSrv @Inject() (
  ...
  def eventToAlert(client: CynoxMispClient, event: Event, organisationId: EntityId): Try[Alert] =
    client
  ...
  def getLastSyncDate(client: CynoxMispClient, mispOrganisation: String, organisations: Seq[Organisation with Entity])(implicit
      graph: Graph
  ): Option[Date] = {
...
//  def convertTag(mispTag: MispTag): Tag = tagSrv.parseString(mispTag.name + mispTag.colour.fold("")(c => f"#$c%06X"))
  ...
  def syncMispEvents(client: CynoxMispClient): Unit =
    client
      .currentOrganisationName
      .fold(
