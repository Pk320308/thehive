package org.thp.cynox.migration
...
object Migrate extends App with MigrationOps {
  val defaultLoggerConfigFile = "/etc/cynox/logback-migration.xml"
  ...
  val argParser = {
    ...
    OParser.sequence(
      ...
      version('v', "version"),
      help('h', "help"),
      head("Cynox migration tool", getVersion),
  ...
  OParser.parse(argParser, args, defaultConfig).foreach { config =>
    implicit val actorSystem: ActorSystem = ActorSystem("CynoxMigration", config)
