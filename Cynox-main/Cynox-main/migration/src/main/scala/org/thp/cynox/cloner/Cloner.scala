...
import org.thp.cynox.migration.Migrate.getVersion
...
object Cloner extends App with IntegrityCheckApp {
  ...
  val defaultLoggerConfigFile = "/etc/cynox/logback-cloner.xml"
  ...
  val argParser = {
    ...
    OParser.sequence(
      ...
      version('v', "version"),
      help('h', "help"),
      head("Cynox cloner tool", getVersion),
  ...
  OParser.parse(argParser, args, defaultConfig).foreach { config =>
    implicit val actorSystem: ActorSystem = ActorSystem("CynoxCloner", config)
    ...
    try {
      ...
      val cynoxSchema = new CynoxSchemaDefinition
      val cortexSchema  = new CortexSchemaDefinition

      {
        val expectedVersion = cynoxSchema.operations.operations.length + 1
        val foundVersion    = sourceDatabase.version(cynoxSchema.name)
        if (foundVersion != expectedVersion) {
          println(s"The schema of Cynox is not valid (expected: $expectedVersion, found: $foundVersion)")
      ...
      val models = destDatabase.extraModels ++ cynoxSchema.modelList ++ cortexSchema.modelList
      destDatabase.createSchema(models)
      destDatabase.setVersion(cynoxSchema.name, sourceDatabase.version(cynoxSchema.name))
