package org.thp.cynox.migration.th4
...
import org.thp.cynox.connector.cortex.models.{CortexSchemaDefinition, CynoxCortexSchemaProvider}
import org.thp.cynox.connector.cortex.services.{ActionSrv, JobSrv}
import org.thp.cynox.dto.v1.InputCustomFieldValue
import org.thp.cynox.migration.IdMapping
import org.thp.cynox.migration.dto._
...
import org.thp.cynox.{migration, ClusterSetup}
        ...
        (play.api.inject.guice.GuiceableModule.guiceable(new EhCacheModule).guiced(Environment.simple(), configuration, Set.empty) :+
          new ScalaModule with AkkaGuiceSupport {
            override def configure(): Unit = {
              ...
              bind[Schema].toProvider[CynoxCortexSchemaProvider]
...
class Output @Inject() (
    ...
    cynoxSchema: CynoxSchemaDefinition,
...
//    tagSrv: TagSrv,
  ...
  override def startMigration(): Try[Unit] = {
    ...
    if (resumeMigration)
      db.addSchemaIndexes(cynoxSchema)
      ...
      db.setVersion(cynoxSchema.name, cynoxSchema.operations.lastVersion)
