...
import org.thp.cynox.connector.cortex.models._
import org.thp.cynox.connector.cortex.services.ActionOps._
import org.thp.cynox.connector.cortex.services.Conversion._
...
import org.thp.cynox.services.{LogSrv, OrganisationSrv}
...
class ActionSrv @Inject() (
  ...
  def execute(entity: Product with Entity, cortexId: Option[String], workerId: String, parameters: JsObject)(implicit
    ...
    for {
      ...
      inputCortexAction =
        CortexAction(label, writes.writes(entity), s"cynox:${fromObjectType(entity._label)}", tlp, pap, parametersWithRequesterInfo)
