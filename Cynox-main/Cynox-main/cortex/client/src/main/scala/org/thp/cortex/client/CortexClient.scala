...
class CortexClient(
    val name: String,
    val baseUrl: String,
    val includedCynoxOrganisations: Seq[String],
    val excludedCynoxOrganisations: Seq[String],
  ...
  def getRespondersByType(entityType: String): Future[Seq[OutputWorker]] =
    responder
      .search[SearchQuery](SearchQuery(field = "dataTypeList", value = s"cynox:$entityType", range = "all"))
